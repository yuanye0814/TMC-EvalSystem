...
void __attribute__ ((interrupt)) EXTI5_9_IRQHandler(void)
{
	...
	if(GET_BITS(AFIO_EXTISS2,0,3) == 3){
		PD8_IRQHandler(); // For TMC6140-eval diagnostics
	}
	else if(GET_BITS(AFIO_EXTISS2,0,3) == 2 || GET_BITS(AFIO_EXTISS1,12,15) == 2){
...
void IDDetection_init(void)
{
	...
	gpio_init(HAL.IOs->pins->ID_CH0.port, GPIO_MODE_IPU, GPIO_OSPEED_50MHZ, HAL.IOs->pins->ID_CH0.bitWeight);

	//syscfg_exti_line_config(EXTI_SOURCE_GPIOC, EXTI_SOURCE_PIN8);
	gpio_exti_source_select(GPIO_PORT_SOURCE_GPIOC, GPIO_EVENT_PIN_8);
	...
	gpio_init(HAL.IOs->pins->ID_CH1.port, GPIO_MODE_IPU, GPIO_OSPEED_50MHZ, HAL.IOs->pins->ID_CH1.bitWeight);

	//syscfg_exti_line_config(EXTI_SOURCE_GPIOC, EXTI_SOURCE_PIN7);
	gpio_exti_source_select(GPIO_PORT_SOURCE_GPIOC, GPIO_EVENT_PIN_7);
...
static int32_t detectID_EEPROM(IdAssignmentTypeDef *ids)
{
	...
	if(ids->ch2.state != ID_STATE_DONE)
	{
		...
		if(!eeprom_check(&SPI.ch2))
		{
			...
			#if 1 // TMC2240 test ID=28
			ids->ch2.id = ID_TMC2240;
