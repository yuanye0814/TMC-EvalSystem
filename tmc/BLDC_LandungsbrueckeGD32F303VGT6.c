/*
 * @Author       : Alvin
 * @Date         : 2023-09-23 22:06:27
 * @LastEditTime : 2023-09-25 22:41:09
 * @LastEditors  : Alvin
 * @FilePath     : \TMC-EvalSystem\tmc\BLDC_LandungsbrueckeGD32F303VGT6.c
 * @Description  :
 */
/*
 * BLDC.c
...
static HallStates inputToHallState(uint8_t in_0, uint8_t in_1, uint8_t in_2)
{
	...
	{
	case 0: // U/V/W
		retVal = in_0 << 0 | in_1 << 1 | in_2 << 2;
		break;
	case 1: // V/W/U
		retVal = in_0 << 1 | in_1 << 2 | in_2 << 0;
		break;
	case 2: // W/U/V
		retVal = in_0 << 2 | in_1 << 0 | in_2 << 1;
		break;
	}
...
void BLDC_init(BLDCMeasurementType type, uint32_t currentScaling, IOPinTypeDef *hallU, IOPinTypeDef *hallV, IOPinTypeDef *hallW)
{
	...
	adc_regular_channel_config(ADC1, 0, adcPhases[adc], ADC_SAMPLETIME_13POINT5);
...
void ADC0_1_IRQHandler()
{
	...
	{
		uint16_t tmp = adc_regular_data_read(ADC1);
...
void BLDC_enablePWM(uint8_t enable)
{
	if (enable)
	{
		...
		Pins.PWM_UH->configuration.GPIO_Mode = GPIO_MODE_AF_PP;
		Pins.PWM_UH->configuration.GPIO_OType = GPIO_MODE_AF_PP;
		Pins.PWM_UH->configuration.GPIO_PuPd = GPIO_MODE_AF_PP;
	...
	{
		Pins.PWM_UH->configuration.GPIO_Mode = GPIO_MODE_IPU;
		Pins.PWM_UH->configuration.GPIO_OType = GPIO_MODE_IPU;
		Pins.PWM_UH->configuration.GPIO_PuPd = GPIO_MODE_IPU;
