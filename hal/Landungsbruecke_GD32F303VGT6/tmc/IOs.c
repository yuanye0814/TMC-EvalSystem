...
static void setPinConfiguration(IOPinTypeDef *pin)
{
	...
	gpio_init(pin->port, pin->configuration.GPIO_Mode, pin->configuration.GPIO_Speed, pin->bitWeight);
}
...
static void setPin2Input(IOPinTypeDef *pin)
{
	...
	pin->configuration.GPIO_Mode = GPIO_MODE_IN_FLOATING;
	setPinConfiguration(pin);
}
...
static void setPinState(IOPinTypeDef *pin, IO_States state)
{
	...
	case IOS_HIGH:
		...
		*pin->setBitRegister = pin->bitWeight;
		break;
	case IOS_OPEN:
		pin->configuration.GPIO_Mode = GPIO_MODE_AIN;
		break;
	default:
		break;
	}
...
static IO_States getPinState(IOPinTypeDef *pin)
{
	...
	if ((pin->configuration.GPIO_Mode == GPIO_MODE_IN_FLOATING) ||
		(pin->configuration.GPIO_Mode == GPIO_MODE_IPD) || (pin->configuration.GPIO_Mode == GPIO_MODE_IPU))
		pin->state = (GPIO_ISTAT(pin->port) & pin->bitWeight) ? IOS_HIGH : IOS_LOW;
	else if ((pin->configuration.GPIO_Mode == GPIO_MODE_OUT_OD) || (pin->configuration.GPIO_Mode == GPIO_MODE_OUT_PP))
